	...
	public:
		...
		tuple<pair<int, int>, pair<int, int>> triangulate(tuple<double, double, double> circle1, tuple<double, double, double> circle2) {
			...
			if (abs(d - (r1 + r2)) < 1 || abs(d - std::abs(r1 - r2)) < 1)  {
				...
				pair<int, int> intersection1 = make_pair(static_cast<int>(x0 + h * (y2 - y1) / d), static_cast<int>(y0 - h * (x2 - x1) / d));
				pair<int, int> intersection2 = make_pair(static_cast<int>(x0 - h * (y2 - y1) / d), static_cast<int>(y0 + h * (x2 - x1) / d));
		...
		void UIscreen(){
			...
			auto intersections = triangulate(circle1, circle2);

...
class RangeDoppler : public RadarBlock
{
	...
	void getADCaverage(int index_1D, complex<float>* xnvalues, complex<float>* adc_data) {
	    ...
	    const int range_bin = (index_1D % FAST_TIME) - 1;
	    for(int i=0; i<TX*RX; i++) {
			complex<float> sum = 0;
			complex<float> sixtyfour(64,0);
			for(int j=range_bin; j<RD_bins; j+=FAST_TIME) {
				sum = sum + adc_data[i*RD_bins + j];
			}
			complex<float> avg = sum / sixtyfour;
			xnvalues[i] = avg;
		}
...
*/
	...
	void shape_angle_data(float* rdm_avg, float* prev_rdm_avg, float* cfar_cube, std::complex<float>* adc_data, std::complex<float>* angle_data, int* cfar_max) {
	    ...
	    correlation_matrix(zero_rdm_avg, prev_rdm_avg, cfar_cube, adc_data, cfar_max, Rmatrix, final_range);
	    compute_angle_est();
